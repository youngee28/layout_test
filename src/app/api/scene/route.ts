    ...
    blocks: {
      ...
      items: {
        ...
        properties: {
          ...
          title: { type: "string" },
          message: { type: "string" },
          chartType: { type: "string", enum: ["bar", "line", "donut", "pie", "kpi", "rankingBar", "comboBarLine"] },
...
function buildDashboardSpecPrompt({ tables }: { tables: ResolvedTable[] }): string {
  return [
    ...
    "현재 안정적으로 렌더링 가능한 chartType은 bar, rankingBar, line, comboBarLine입니다.",
    ...
    "comboBarLine chart block은 반드시 dataBinding.tableId, barValueField, lineValueField, 그리고 dateField 또는 categoryField 중 하나를 포함해야 합니다.",
    "comboBarLine은 같은 x축을 공유하고, barValueField는 막대로 표현할 절대값 지표, lineValueField는 선으로 표현할 추세/비율/보조 지표일 때만 사용하세요.",
    "comboBarLine의 두 지표는 같은 tableId의 같은 row 기준으로 정렬 가능해야 하고, 함께 볼 때 해석 가치가 있어야 합니다.",
